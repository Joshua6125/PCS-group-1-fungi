...
def main():
    ...
    print(simulation.get_grid_representation(show_toxins=sim_parameters["show_toxins"]))
    for _ in range(100):
        time.sleep(0.4)
        simulation.step()
        sim_string = simulation.get_grid_representation(show_toxins=sim_parameters["show_toxins"])
        print(sim_string)
